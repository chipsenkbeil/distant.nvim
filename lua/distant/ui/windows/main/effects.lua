...
return {
    --- @type {[string]: distant.plugin.ui.windows.main.Effect}
    kinds = EFFECTS,

    --- @type {[distant.plugin.ui.windows.main.Effect]: fun(event:distant.core.ui.window.EffectEvent)}
    handlers = {
        [EFFECTS.CLOSE_WINDOW] = function(event)
            event.window:close()
        end,

        [EFFECTS.KILL_CONNECTION] = function(event)
        ...
        [EFFECTS.LAUNCH_SERVER] = function(event)
        ...
        [EFFECTS.TOGGLE_EXPAND_CONNECTION] = function(event)
            --- @type {id:distant.core.manager.ConnectionId, destination:distant.core.Destination}
            local payload = event.payload
            local id = payload.id
        ...
        [EFFECTS.TOGGLE_EXPAND_CURRENT_SETTINGS] = function(event)
            local window = event.window
        ...
        [EFFECTS.RELOAD_TAB] = function(event)
        ...
        [EFFECTS.SET_VIEW] = function(event)
            --- @type string
        ...
        [EFFECTS.SWITCH_ACTIVE_CONNECTION] = function(event)
            --- @type {id:distant.core.manager.ConnectionId, destination:distant.core.Destination}
            local payload = event.payload
            local id = payload.id
