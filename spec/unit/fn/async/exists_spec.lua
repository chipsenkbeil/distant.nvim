...
describe('fn.exists (async)', function()
    it('should send an exists request via the global client', function()
        local send = spy.new(function() end)
        u.stub_send(send)

        local path = 'some/path'
        fn.async.exists(path, function() end)
    ...
    it('should invoke the callback with appropriate result on success', function()
        ...
        fn.async.exists('some/path', function(err, data)
            assert.is.falsy(err)
