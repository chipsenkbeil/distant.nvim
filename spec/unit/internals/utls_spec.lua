...
describe('internal.utils', function()
   ...
   describe('merge', function()
      it('should produce an empty table if given no arguments', function()
         assert.are.same({}, u.merge())
      end)

      it('should produce a deep clone of a singular table', function()
         local tbl = {a = 3}
         local tbl2 = u.merge(tbl)

         assert.are.same({a = 3}, tbl)
         assert.are.same({a = 3}, tbl2)

         tbl2.a = 4
         assert.are.same({a = 3}, tbl)
         assert.are.same({a = 4}, tbl2)
      end)

      it('should merge fields deeply from left to right', function()
         -- Replace equivalent type
         assert.are.same({a = 4}, u.merge({a = 3}, {a = 4}))

         -- Don't override with nil value
         assert.are.same({a = 3, b = 4}, u.merge({a = 3}, {a = nil, b = 4}))

         -- Support overriding one type with another
         assert.are.same(
            {a = 3, b = {c = 'hello'}},
            u.merge({a = 3, b = 4}, {b = {c = 'hello'}})
         )

         -- Support merging nested tables
         assert.are.same(
            {a = 3, b = {c = 4, d = 'hello'}},
            u.merge({a = 3, b = {c = 1, d = 'hello'}}, {b = {c = 4}})
         )
      end)
   end)
